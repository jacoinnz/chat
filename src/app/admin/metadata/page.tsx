...
export default function MetadataPage() {
    ...
    async function load() {
      try {
        ...
        if (response.ok) {
          ...
          setTaxonomy(data.taxonomy as Taxonomy);
        }
      } catch {
        // Use defaults
      } finally {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (response.ok) {
        setMessage({ type: "success", text: "Taxonomy saved successfully" });
      } else {
        const data = await response.json();
        setMessage({ type: "error", text: data.error || "Failed to save" });
      }
    } catch {
      setMessage({ type: "error", text: "Failed to save taxonomy" });
    } finally {
      setSaving(false);
    }
  };
    ...
    <div className="space-y-6 max-w-2xl">
      ...
      <div className="space-y-6">
        <section className="bg-white rounded-lg border border-[#d0d8e0] p-4">
          <h3 className="text-sm font-medium text-[#1a2a3a] mb-3">Department</h3>
          <EditableList
        ...
        <section className="bg-white rounded-lg border border-[#d0d8e0] p-4">
          <h3 className="text-sm font-medium text-[#1a2a3a] mb-3">Sensitivity</h3>
          <EditableList
        ...
        <section className="bg-white rounded-lg border border-[#d0d8e0] p-4">
          <h3 className="text-sm font-medium text-[#1a2a3a] mb-3">Status</h3>
          <EditableList
